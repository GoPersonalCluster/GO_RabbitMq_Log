...
type ErrorLogStrategy struct {
	event *consumer.IntegrationEvent
}

func (pQS *ErrorLogStrategy) New(iE *consumer.IntegrationEvent) (consumer.StrategyHandler, error) {
	iE.EventName = "PII"
	mh := iE.CreateMetaHeader(config.GetHostName(), "ErrorLogEvent")
	iE.MetaHeader = append(iE.MetaHeader, mh)

	return &ErrorLogStrategy{event: iE}, nil
}
